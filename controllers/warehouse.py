...
class WsAuth(http.Controller):
    ...
    def create_warehouse(self, **post):
        ...
        shelf = request.env['warehouse.shelf'].sudo().search([('row', '=', post.get('row')),
                                                              ('bay', '=', post.get('bay'))], limit=1)
    ...
    def put_product_shelf(self, **put_data):
        ...
        try:
            ...
            warehouse.sudo().write({'quantity': warehouse.quantity - 1 if warehouse.quantity > 0 else 0})
